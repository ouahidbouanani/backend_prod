...
export const getActivities = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const rows = lotsExclus.length > 0
      ? await prisma.$queryRaw<{ activity: string | null }[]>(Prisma.sql`
          ...
          WHERE de.id_lot NOT IN (${Prisma.join(lotsExclus)})
...
export const getTypePiecesOptions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const rows = lotsExclus.length > 0
      ? await prisma.$queryRaw<{ type_pieces: string | null }[]>(Prisma.sql`
...
export const getLots = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const debutRows = lotsExclus.length > 0
      ? await prisma.$queryRaw<{ id_lot: number; nb_passage: number }[]>(Prisma.sql`
    ...
    const prises = await prisma.prise_de_cotes.findMany({
      select: {
        id_lot: true,
        nb_passage: true,
      },
    ...
    const prisesSet = new Set(
      prises.map((p) => `${p.id_lot}-${p.nb_passage}`)
    );

    // Map : id_lot -> passage max déjà mesuré
    const maxPassageMap = new Map<number, number>();
    ...
    const fins = await prisma.fin_impression.findMany({
      select: {
        id_lot: true,
        nb_imprimees: true,
      },
    ...
    const finMap = new Map(
      fins.map((f) => [f.id_lot, f.nb_imprimees] as const)
    );

    // 6. Regrouper par lot + liste des passages
    const groupedObj: Record<
      number,
      { id_lot: number; nb_passages: number[]; nb_imprimees: number | null }
    > = {};
...
export const submitPieces = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const GetTypePiece = async (req: Request, res: Response): Promise<Response | void> => {
    try {
...
export const ajouterPriseCotes = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const getMesuresByLotAndPassage = async (req: Request, res: Response): Promise<void> => {
    try {
