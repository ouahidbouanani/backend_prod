 ...
 * }
 */
export const createDebutTomoFinis = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const finExists = await prisma.fin_assemblage.findUnique({
		...
		const already = await prisma.debuttomo_finis.findUnique({
			where: { id_debuttomo },
		...
		const nbDisponibles = await prisma.fin_assemblage_piece.count({
			where: { id_finassemblage: id_debuttomo },
		});
		...
		const created = await prisma.debuttomo_finis.create({
			data: {
				id_debuttomo,
				activite,
				produit_fini,
 ...
 */
export const getDebutTomoFinisById = async (req: Request, res: Response): Promise<void> => {
	try {
		...
		const row = await prisma.debuttomo_finis.findUnique({
