...
function createOpenApiSpec(options = {}) {
    const port = process.env.PORT || 3000;
    const baseUrl = options.baseUrl || process.env.SWAGGER_BASE_URL || `http://localhost:${port}`;
    const definition = {
        openapi: '3.0.3',
        info: {
            title: 'FrogBox API',
            version: process.env.npm_package_version || '1.0.0',
            description: 'Documentation Swagger/OpenAPI du backend FrogBox. Utilise les annotations JSDoc dans les fichiers de routes.',
        },
        servers: [{ url: baseUrl }],
        tags: [
