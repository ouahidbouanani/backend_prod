...
exports.getFinAssemblageIds = exports.getFinAssemblageById = exports.createFinAssemblage = void 0;
 ...
 * }
 */
const createFinAssemblage = async (req, res) => {
    try {
        ...
        const debut = await prisma_1.default.debut_assemblage.findUnique({
        ...
        const already = await prisma_1.default.fin_assemblage.findUnique({
        ...
        const allowed = await prisma_1.default.debut_assemblage_piece.findMany({
            where: { id_debutassemblage: finId },
...
exports.createFinAssemblage = createFinAssemblage;
/**
 * GET /api/finassemblage/:id
 */
const getFinAssemblageById = async (req, res) => {
    try {
        const id = req.params.id;
        const row = await prisma_1.default.fin_assemblage.findUnique({
...
exports.getFinAssemblageById = getFinAssemblageById;
/**
 * GET /api/finassemblage/ids
 */
const getFinAssemblageIds = async (_req, res) => {
    try {
        // Exclure les dossiers déjà déclarés dans `debuttomo_finis`
        // (donc non disponibles pour une nouvelle déclaration).
        const alreadyDeclared = await prisma_1.default.debuttomo_finis.findMany({
            select: { id_debuttomo: true },
        });
        const declaredIds = alreadyDeclared.map((r) => r.id_debuttomo);
        const ids = await prisma_1.default.fin_assemblage.findMany({
...
exports.getFinAssemblageIds = getFinAssemblageIds;
